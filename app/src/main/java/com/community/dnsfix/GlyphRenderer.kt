...
class GlyphRenderer(private val baseInkColor: Int) {
    ...
    fun drawWord(
        ...
        if (rawPath.isEmpty) {
            // Fallback for fonts where getTextPath returns empty (some Myanmar clusters)
            for (i in 0..3) {
                val offX = PenState.gaussian(0f, 0.4f, washRand)
                val offY = PenState.gaussian(0f, 0.4f, washRand)
            ...
            for (i in 0..7) {
                val offX = PenState.gaussian(0f, 0.6f, washRand)
                val offY = PenState.gaussian(0f, 0.6f, washRand)
